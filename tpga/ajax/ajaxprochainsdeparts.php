...
<div class="list-block media-list list-departures">
    <ul>
        ...
        <?php foreach ($nextDepartures->departures as $depart) { ?>
            <?php
//            echo $depart->connectionWaitingTime; // NotSupported?
            ...
            ?>
                <div class="item-inner">
                    <div class="item-title-row">
                        <div class="item-title"><?= $depart->connection->lineCode ?> ➜ <?= Stops::format($depart->connection->destinationName) ?>
                            <?= $depart->connection->lineCode == 47 ? '↻' : '' ?>
                            <?= $depart->connection->lineCode == 48 ? '↺' : '' ?></div>
